...
export default function NotificationPage() {
  ...
  useEffect(() => {
    if (currentUser && token) {
      const shown = sessionStorage.getItem("threads_welcome_shown");
      if (!shown) { setTimeout(() => { toast.success(`Selamat datang di Threads! 🧵`, { description: `Halo ${currentUser.name}, selamat menikmati aktivitas terbaru.`, duration: 5000 }); sessionStorage.setItem("threads_welcome_shown", "1"); }, 500); }
    }
  }, [currentUser, token]);
